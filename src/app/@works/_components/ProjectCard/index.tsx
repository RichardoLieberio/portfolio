    ...
    return (
        <li tabIndex={ 0 } role="listitem" className="w-full max-w-xl md:max-w-none h-full mx-auto relative bg-white border border-[var(--border)] rounded-xl overflow-hidden transition hover:border-[var(--primary)] focus-visible:outline-2 focus-visible:outline-[var(--primary)]">
            ...
            <div className="px-8 pt-6 pb-18 md:pb-20 border-t border-[var(--border)]">
                <p className="w-fit flex items-center gap-2 font-semibold">{ title }</p>
                ...
                <div className="mt-6 flex flex-wrap items-center gap-x-2 gap-y-4">
                    { stack.map(({ name, logo, small }: { name: string, logo: string, small?: boolean }, index) => (
                        <div key={`Stack ${index} - ${name}`} className="px-3 py-1 flex items-center gap-2 bg-gray-100 rounded-lg">
