...
import { useState, useEffect, JSX, Dispatch, SetStateAction } from 'react';
import { motion, animate, useMotionValue, useTransform, Easing, MotionValue, AnimationPlaybackControls } from 'framer-motion';
...
export default function Number({ number, duration, ease }: NumberProps): JSX.Element {
    ...
    }, [ number, duration, ease, count, rounded ]);

    return (
        <motion.span className="text-4xl md:text-5xl font-semibold" aria-live="polite" aria-label={ `Count: ${displayNumber}` }>
            { displayNumber }+
        </motion.span>
    );
}
